...
namespace FunctionalProgramming.Monad.Parsing
{
    ...
    public interface IParser<TInput, TOutput>
    {
        ...
        IParser<TInput, IEnumerable<TOutput>> Repeat(Repititions r);

        IParser<TInput, IEnumerable<TOutput>> Repeat<T>(Repititions r, IParser<TInput, T> delimiter);  
    ...
    public class ParseResultParser<TInput, TOutput, TResult> : Parser<TInput, TResult>
    {
        ...
        {
            return _p1.Apply(reader).Match(
                success: (val1, r1) => _p2.Apply(new[] { val1 }).Match<IParseResult<TInput, TResult>>(
    ...
    public abstract class Parser<TInput, TOutput> : IParser<TInput, TOutput>
    {
        ...
        public IParser<TInput, IEnumerable<TOutput>> Repeat(Repititions r)
        {
            return new RepeatingParser<TInput, TOutput>(this, r);
        }

        public IParser<TInput, IEnumerable<TOutput>> Repeat<T>(Repititions r, IParser<TInput, T> delimiter)
        {
    ...
    public class RepeatingParser<TInput, TOutput> : Parser<TInput, IEnumerable<TOutput>>
    {
        ...
        private IParseResult<TInput, IEnumerable<TOutput>> Apply(IEnumerable<TInput> reader, Repititions r)
        {
            return r.Match(
                        ...
                        (v, rest) =>
                            Apply(rest, r)
                                .Match(
                                    success:
                                        (vs, remaining) =>
                                            new SuccessResult<TInput, IEnumerable<TOutput>>(v.Cons(vs), remaining),
                                    failure:
                                        (error, remaining) =>
                                            new SuccessResult<TInput, IEnumerable<TOutput>>(v.LiftEnumerable(),
                                                remaining)),
                    failure:
                        (e, rest) => new SuccessResult<TInput, IEnumerable<TOutput>>(Enumerable.Empty<TOutput>(), rest)),
                oneOrMoreReps: () => _parser.Apply(reader).Match<IParseResult<TInput, IEnumerable<TOutput>>>(
                    success:
                        (v, rest) =>
                            Apply(rest, r)
                                .Match(
                    ...
                    failure:
                        (e, rest) =>
                            new FailureResult<TInput, IEnumerable<TOutput>>(
                                string.Format("Expected one or more of ({0}) but got {1} instead", _parser.ToString(), e),
                                rest)),
                nReps:
                    n =>
                        F.If(n <= 0,
                            () => new SuccessResult<TInput, IEnumerable<TOutput>>(Enumerable.Empty<TOutput>(), reader),
                            () => _parser.Apply(reader).Match(
                                success: (v, rest) => Apply(rest, new NRepititions(n - 1))
                                    .Match<IParseResult<TInput, IEnumerable<TOutput>>>(
                                        success:
                                            (vs, remaining) =>
                                                new SuccessResult<TInput, IEnumerable<TOutput>>(v.Cons(vs), remaining),
                                        failure:
                                            (e, remaining) =>
                                                new FailureResult<TInput, IEnumerable<TOutput>>(e, remaining)),
                                failure:
                                    (e, rest) =>
                                        new FailureResult<TInput, IEnumerable<TOutput>>(
